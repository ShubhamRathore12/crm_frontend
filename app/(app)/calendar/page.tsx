                  ...
                  <div className="space-y-1">
                    {dayLeads.slice(0, 3).map((lead) => (
                      <div key={lead.id} className="text-[10px] p-1.5 rounded bg-background border shadow-sm truncate hover:border-primary/40 hover:shadow-md transition-all cursor-pointer">
                        <span className="font-semibold text-primary/70">Lead:</span> {lead.contact_name || lead.source}
                      </div>
                    ))}
                    {dayLeads.length > 3 && (
            ...
            <CardContent className="px-0 space-y-4">
              {leads.filter(l => new Date(l.created_at) > currentDate).slice(0, 5).map(lead => (
                <div key={lead.id} className="p-4 rounded-xl border bg-card/50 hover:border-primary/30 transition-all hover:bg-card shadow-sm hover:shadow-md group">
                  ...
                  <p className="text-xs text-muted-foreground line-clamp-1 mt-0.5">{lead.contact_name || "New inbound lead"}</p>
                </div>
              ))}
